...
pacman::p_load(tidySEM, haven, tidyverse, future, progressr, gt, nnet, OpenMx, gtsummary, mice, here, emmeans, patchwork)
set.seed(152738)
options(mc.cores = 8)
plan(multisession) 
handlers("progress") 
...
```{r}
selected_vars <- c("sex", "gender", "age", "education", "racem", "income", "sexualidentity", "urbanicity")
dem_data <- subset(data, select = selected_vars)

# Ensure all variables are properly formatted
dem_data <- dem_data %>%
  mutate(across(c(sex, gender, education, income, racem, sexualidentity, urbanicity), as_factor),
         age = as.numeric(age))
...
gender_df <- dem_data %>%
  ...
  mutate(variable = "Gender Identity")

race_df <- dem_data %>%
  count(category = racem) %>%
  mutate(variable = "Racial/Ethnic Identity")

sexual_df <- dem_data %>%
  count(category = sexualidentity) %>%
  mutate(variable = "Sexual Identity")

# Bind and create group_index within each variable for coloring
plot_df <- bind_rows(gender_df, race_df, sexual_df) %>%
  group_by(variable) %>%
  arrange(variable, desc(n)) %>%
  mutate(
    percent = n / sum(n) * 100,
    label = paste0(category, " (", round(percent, 1), "%)"),
    group_id = row_number()  # This determines the color within the column
  ) %>%
  ungroup()

# Define a palette (e.g. 6 colors, repeated for each column)
my_palette <- c("#F8766D", "#B79F00", "#00BA38", "#00BFC4", "#619CFF", "#F564E3")

dem_fig <- ggplot(plot_df, aes(x = variable, y = n, fill = factor(group_id))) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = label), position = position_stack(vjust = 0.5), color = "white", size = 12) +
...
saveRDS(dem_fig, file = here("analysis", "figures", "dem_plot.Rds"))
```
...
lpa_fig <- lpa1_plot + 
  theme(aspect.ratio = NULL)  # This removes any fixed aspect ratio

saveRDS(lpa_fig, file = here("analysis", "figures", "lpa_plot.Rds"))
```
...
ggplot(lpa1_data, aes(class, fill = as_factor(racem))) +
         geom_bar() +
  labs(fill = "Race/Ethnicity") +
  theme(
    legend.position = "bottom",
    legend.direction = "vertical",
    legend.title.align = 0.5  # optional: centers the legend title
  ) +  
ggplot(lpa1_data, aes(class, fill = as_factor(sexualidentity))) +
         geom_bar() + 
  labs(fill = "Sexual Identity") +
  theme(
    legend.position = "bottom",
    legend.direction = "vertical",
    legend.title.align = 0.5  # optional: centers the legend title
...
### Figures for Predictors

#### Gender Identity

```{r}
gender_lpa_plot <- ggplot(lpa1_data, aes(class, fill = haven::as_factor(gender))) +
  geom_bar() +
  geom_text(aes(label = haven::as_factor(gender)), 
            stat = "count",
  ...
  theme(
    legend.position = "none"  # additional insurance to remove legend
  )

saveRDS(gender_lpa_plot, file = here("analysis", "figures", "gender_lpa_plot.Rds"))
```

#### Race

```{r}
racem_lpa_plot <- ggplot(lpa1_data, aes(class, fill = haven::as_factor(racem))) +
  geom_bar() +
  geom_text(aes(label = haven::as_factor(racem)), 
            stat = "count",
            position = position_stack(vjust = 0.5),
            color = "white",
            fontface = "bold",
            size = 6) +
  labs(title = "Racial/Ethnic Identity") + 
  guides(fill = "none") +  # removes the fill legend
  theme(
    legend.position = "none"  # additional insurance to remove legend
  )

saveRDS(racem_lpa_plot, file = here("analysis", "figures", "racem_lpa_plot.Rds"))
```

### Sexual Orientation

```{r}
sexid_lpa_plot <- ggplot(lpa1_data, aes(class, fill = haven::as_factor(sexualidentity))) +
  geom_bar() +
  geom_text(aes(label = haven::as_factor(sexualidentity)), 
            stat = "count",
            position = position_stack(vjust = 0.5),
            color = "white",
            fontface = "bold",
            size = 6) +
  labs(title = "Sexual Identity") + 
  guides(fill = "none") +  # removes the fill legend
  theme(
    legend.position = "none"  # additional insurance to remove legend
  )

saveRDS(sexid_lpa_plot, file = here("analysis", "figures", "sexid_lpa_plot.Rds"))

saveRDS(lpa1_data, file = here("data", "lpa_data.Rds"))
```
